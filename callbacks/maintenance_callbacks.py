...
def register(app):
    ...
    @app.callback(Output("maintenance-kpi-grid", "children"), Input("maintenance-interval", "n_intervals"))
    def _kpis(_n):
        ...
        return [
            kpi_card("Equipment Online", f"{m['online']}/{m['total']}", icon_letter="⚙", accent=Colors.ACCENT),
            kpi_card("Equipment Warning", str(m["warning"]), icon_letter="▲", accent=Colors.WARNING),
            kpi_card("Critical Equipment", str(m["critical"]), icon_letter="!", accent=Colors.CRITICAL),
            kpi_card("Average Health Score", f"{avg_health:.1f}%", icon_letter="◈", accent=Colors.SUCCESS),
            kpi_card("Active Alerts", str(active_alerts), icon_letter="◉", accent=Colors.ACCENT_PURPLE),
        ]
    ...
    def _equipment_grid(_n, selected_id):
        return [equipment_card(eq, selected=(eq.id == selected_id)) for eq in ENGINE.equipment]
    ...
    def _digital_view(_n, selected_id):
        ...
        for label, value in rows:
            table_rows.append(
                html.Div(
                    [html.Div(label, className="text-muted", style={"fontSize": "12px", "flex": "1"}),
        ...
        return [
            html.Div(
                [html.Div("Equipment Digital View", className="panel-title"),
                 html.Span(eq.zone, className="panel-tag")],
    ...
    def _recommendation(_n, selected_id):
        eq = ENGINE.maintenance.equipment_by_id(selected_id) or ENGINE.equipment[0]
        ...
        return html.Div(
            [
                html.Div([severity_badge(sev, label=title)], style={"marginBottom": "10px"}),
                html.Div(message, style={"fontSize": "13px", "color": "var(--text-secondary)", "marginBottom": "10px", "lineHeight": "1.6"}),
                html.Div(
                    [html.Span("Recommended action: ", style={"color": "var(--accent)", "fontWeight": 700}), rec],
                ...
                html.Div(
                    [
                        html.Span(f"Priority: {priority}", className="mono", style={"marginRight": "16px"}),
                        html.Span(f"Anomaly Confidence: {eq.anomaly_confidence:.0f}%", className="mono"),
                    ],
    ...
    def _temp_chart(_n, selected_id):
        eq = ENGINE.maintenance.equipment_by_id(selected_id) or ENGINE.equipment[0]
        th = eq.thresholds()
    ...
    def _health_risk_chart(_n, selected_id):
        eq = ENGINE.maintenance.equipment_by_id(selected_id) or ENGINE.equipment[0]
        x = list(range(len(eq.health_history)))
        series = [
            {"x": x, "y": list(eq.health_history), "name": "Health Score", "color": Colors.SUCCESS},
            {"x": x, "y": list(eq.risk_history), "name": "Risk Score", "color": Colors.CRITICAL},
        ]
